...
/**
 * 实现对response对象的后处理，加入自定义功能(修饰器模式)。
 * @author JiangBo
 *
 */
public class ScopeHttpServletResponseWrapper extends HttpServletResponseWrapper {
    ...
    public ScopeHttpServletResponseWrapper(HttpServletRequest request, HttpServletResponse httpServletResponse,
            String scopeCode) {
        super(httpServletResponse);
        this.contextPath = request.getContextPath();
        this.prefix = contextPath + "/" + scopeCode;
        this.absolutePrefix = request.getScheme() + "://" + request.getServerName();
